...
3. **Companion paper, [draft v0.1](./companion/closing-u1-v0.1-DRAFT.md):** *Closing U1: Root Identity by Proximate Cause*. Scope is the material added after the foundation paper was deposited and which resolves limitations that paper states: `RootIdentity.lean` (U1 closed by a doctrine of remoteness), `Responsiveness.lean` (the converse of invariance — a material change must move the verdict), and `Asymmetric.lean` (claims whose falsifier is singular, where comparing root counts is the wrong instrument). Recorded in `ERRATA.md` under `[E9]` until it exists. It must carry the residual with it: the closure defines what a root is and does not promise that a laundered root will be detected.
...
5. **Later architecture paper:** the dual evidence/search ledger remains a research direction until the schemas, conclusion-strength rules, incomplete-coverage controls, matched comparisons, and a real-provider test are preregistered and evaluated.
