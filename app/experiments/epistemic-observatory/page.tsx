...
export default function EpistemicObservatoryPage() {
  return <main>
    ...
    <header className="experiment-hero observatory-page-hero">
      ...
      <div className="experiment-hero-result"><span>WORLD MP-00001</span><strong>03 : 95</strong><small>independent truth / copied falsehood</small><p>$0 model calls in this browser demo</p></div>
    </header>
    <section className="observatory-intro"><p className="section-index">01 / HOW TO READ IT</p><div><h2>Follow the lineage.<br /><em>Count the origins.</em></h2><p>Filter the claims, select a row, and inspect where it came from. The majority looks large only until its copies collapse back to the same source.</p></div><div className="demo-boundary-card"><span>BOUNDARY</span><p>This is a synthetic teaching instrument. The generator knows the ancestry and ground truth. It is not empirical proof that real-world evidence roots are truthful or independent.</p></div></section>
    <section className="dashboard-section observatory-detail"><div className="section-heading"><div><p className="section-index">02 / EPISTEMIC OBSERVATORY</p><h2>World <em>MP-00001</em></h2></div><div className="live"><i /> SYNTHETIC WORLD · SEED 7</div></div><Observatory /><p className="demo-disclaimer">Interactive synthetic demonstration · No LLM or paid model is called by this page.</p></section>
    <section className="observatory-takeaway"><p className="section-index">03 / THE TAKEAWAY</p><h2>Ninety-five votes<br />can still be <em>one claim.</em></h2><p>Lineage changes the unit being counted. Instead of rewarding repetition, the demonstration counts distinguishable evidence origins and preserves uncertainty when provenance is incomplete.</p><a className="preview-link" href="/experiments/capability-tournament">See the measured tournament <span>→</span></a></section>
    <SiteFooter />
  </main>;
}
