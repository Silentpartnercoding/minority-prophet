...
export default function PrivacyPage() {
  return <main className="awe-policy">
    ...
    <section><h2>Deactivate and uninstall</h2><p><code>awe-node uninstall --yes</code> stops collection, removes Agent WEX runtime settings, revokes signing keys, invalidates the API key, and pseudonymizes the account. The account row is marked for purge after 30 days; integrity records may remain under the pseudonymous node ID. Backups are retained locally so uninstall does not destroy pre-existing settings.</p></section>
    <section><h2>Your choices</h2><p>Participation is optional. Inspect with <code>awe-node status</code>, rotate credentials with <code>awe-node rotate-keys</code>, or uninstall at any time. For a privacy issue, open a repository issue without including private data; for a vulnerability, use the private security-reporting path.</p></section>
    <footer><Link href="https://github.com/Silentpartnercoding/minority-prophet/issues">Privacy issue</Link><Link href="/exchange/security">Security reporting</Link></footer>
  </main>;
}
