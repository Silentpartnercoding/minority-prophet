...
export function AweNetworkMotion() {
  ...
  return <div className="awe-network-motion" role="img" aria-label="Animated AWE exchange: an accepted independent failed attempt earns two credits and opens a route search; no credit is spent until a supported route is found; copied evidence folds into its parent; two distinct route candidates form a ranked list; Rank 1 costs one credit and returns to Gate, leaving one credit">
    ...
    <div className="awe-route-lineage-stage">
      <div className="awe-motion-witnesses">
        <article className="awe-root-a"><span>AGENT 27 · 18M AGO</span><b>INDEPENDENT SUCCESS</b><small>root r27 · tool 3.2 + client 1.8</small><em>COPY FOLDED INTO R27</em></article>
        <article className="awe-root-b"><span>AGENT 52 · 29M AGO</span><b>INDEPENDENT SUCCESS</b><small>root r52 · tool 3.2 + client 1.8</small></article>
        ...
        <article className="awe-root-c"><span>AGENT 81 · 10M AGO</span><b>INDEPENDENT SUCCESS</b><small>root r81 · tool 3.3 + client 1.9</small></article>
      </div>
      <div className="awe-route-ranking">
        ...
        {assessment.candidateRoutes.map((candidate) => <article className={candidate.selected ? "awe-ranked-winner" : "awe-ranked-runner-up"} key={`${candidate.toolVersion}-${candidate.clientVersion}`}>
          ...
          <p>{candidate.independentRootCount} independent {candidate.independentRootCount === 1 ? "root" : "roots"} · last seen {minutesBeforeLiveEvaluation(candidate.lastObservedAt)}m ago · valid inside {candidate.evidenceWindowDays}d window</p>
          <b>{candidate.selected ? "SELECTED" : "NEEDS 1 MORE ROOT"}</b>
        </article>)}
      </div>
    </div>

    <p className="awe-candidate-rule">RANK ROUTES · compatible now → independent roots → freshest confirmation · version alone never wins</p>

    <div className="awe-motion-route">
      <span>AWE · ROUTE FOUND · −1 CREDIT</span>
...
export function WorkingRouteDemo() {
  ...
  return <div className="awe-terminal-demo">
    <div className="awe-terminal-copy">
      ...
      <p>AWE finds a supported way around the blocker. The important moment happens in the terminal: the route returns, Gate releases it, and the agent continues.</p>
    </div>

    <div className="awe-terminal-shell" key={cycle} data-replay-cycle={cycle} role="img" aria-label="Automatically looping terminal showing a migration audit fail, contribute its sanitized failure to AWE, receive a working route from independent outcomes, pass Gate, and resume the original task">
      <header><div><i /><i /><i /></div><span>agent@workspace — zsh</span><b>AWE CONNECTED</b></header>
      <div className="awe-terminal-screen">
        ...
        <p className="awe-line awe-line-7"><span className="awe-label">AWE</span> accepted independent failed outcome · root r14 · +2 credits</p>
        ...
        <p className="awe-line awe-line-9"><span className="awe-label">AWE</span> query bound · macos-arm64 · oauth-pkce · ≤7d</p>
        ...
        <p className="awe-line awe-line-13 awe-terminal-dim">[EVIDENCE] {assessment.evidence.successfulIndependentRoots} independent roots support route · {assessment.evidence.copiesCollapsed} dependent root collapsed</p>
        ...
        <p className="awe-line awe-line-22"><span className="awe-label">AWE</span> fresh working-route confirmation submitted · pending verification</p>
...
export function AweCommercialPlayer() {
  ...
  return <div className="awe-commercial-player">
    ...
    <button type="button" onClick={togglePlayback} aria-label={playing ? "Pause AWE demo" : "Play AWE demo"}>
...
export function BackgroundOtelDemo() {
  ...
  return <div className="awe-background-demo">
    <div className="awe-background-copy">
      ...
      <p>Your agent keeps working. Eligible tool outcomes become minimized AWE receipts in the background.</p>
    ...
    <div className="awe-background-stream" key={replay} role="img" aria-label="Background OpenTelemetry flow: operator enables bounded tool-outcome sharing once, an agent works normally, a local adapter removes private content, AWE receives a pending receipt, Minority Prophet checks independence, and a supported route returns through Gate">
      <header><span>AWE / OTEL SIDE STREAM</span><b>OPERATOR BOUND</b></header>
      ...
      <p className="awe-bg-line awe-bg-line-6"><i>AWE</i> pending outcome receipt · 609 bytes in this fixture</p>
