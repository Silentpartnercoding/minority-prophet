...
Every kernel is incomplete until all fields below are committed before its
confirmatory run.
...
Later phases are forbidden until earlier gates pass. Safety-critical kernels
...
environment must reproduce the artifact from the public protocol. The first
interoperability transaction requires two independently written implementations.
