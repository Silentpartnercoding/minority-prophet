...
python3 scripts/run_knowledge_transaction.py \
  research/knowledge-ledger/first-transaction/input.json \
  research/knowledge-ledger/first-transaction/transaction-zero.json
```

`transaction-zero.json` is an executable conformance artifact, not an
experimental finding.

## Bitcoin-style acceptance gate

The first knowledge transaction is complete only when:
