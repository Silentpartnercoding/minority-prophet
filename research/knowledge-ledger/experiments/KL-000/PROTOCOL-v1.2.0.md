...
| 3 | RUN-20260807-9; a **provenance correction, not a rule change** | R5.2's characterisation above — "a decision, not a derivation … could defensibly have gone the other way" — is **corrected: the paper defines the quantity.** `papers/minority-prophet-v1.0.3.md` §3 (line 53): *"The **margin** is \|\|S₁\| − \|S₀\|\|."* — explicitly absolute. The signed reading contradicts the published definition; R5.2 ratified what the paper already said, unknowingly, because no KL-000 document cited the paper. Additionally **F4 is settled for decisive worlds**: Theorem 4 [E2] publishes *"reversal costs ⌊margin/2⌋+1, not margin+1 (4,638/4,638 decisive worlds)"* — the formula R5.1's sub-decision list registered as "reverse-engineered". The margin-0/empty-ledger edge remains outside the theorem's measurement, so the independent implementer's F4 objection stands exactly there and only there. Registered text preserved unrewritten; behaviour unchanged. (Paper finding for the owner, not repaired: Theorem 4's T5-correction writes "\|margin\| > k" — bars on an already-absolute quantity.) | **None.** Behaviour identical; provenance only. |
