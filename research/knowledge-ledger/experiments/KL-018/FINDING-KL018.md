# KL-018 — the registered endpoint passed, and its own control refuted it
...
Not the sequencing. The baseline was built first, which is what caught KL-017.
...
Six failures, one shape: a comparison whose control could not expose the confound.
    ...
    KL-017            a baseline that could not reach the events
    KL-018            a baseline sampling the wrong part of the lifecycle

The first four produced numbers before the flaw surfaced. KL-017 was stopped
before any number existed. KL-018 produced a number and then destroyed it with a
