...
2. **Robustness rule, the method under test:** settles only when
   `assess_dependence_robustness` reports a robust settlement. Otherwise it looks,
   and abstains if looking is unavailable.
3. **Always look:** looks at every decision. This bounds the cost from above.
4. **Oracle (reference):** takes each decision's correct move.
...
- **Primary:**
  - false settlements by the robustness rule in families 1 to 6;
  - the reduction in false settlements against the agreement rule in the families
    where the agreement rule falls.
...
1. **Success criterion.** Proposed:
   - in each of families 1 to 6, the robustness rule makes zero false settlements,
     with the rule-of-three 95% upper bound below the chosen rate (decision 3);
   - it makes significantly fewer false settlements than the agreement rule in
     families 2, 4 and 5.
2. **The cost.** Report extra looks and unneeded abstentions only, or also set a
   ceiling that counts as failure. DRI-2 v2's diagnostic suggests about 2% extra
   looks in single and joint domain and about 70% in shared origin.
...
4. **Imperfect lookups.** Include lookups that return partial or wrong lineage now,
   or keep that for a later version. Including it answers whether looking is
   trustworthy, but mixes two questions in one test.
