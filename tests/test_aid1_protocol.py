...
from experiments.aid1.arms import ARMS, PRIMARY, declared_only_independent_for, run_decision
from experiments.aid1.scoring import evaluate, evaluate_criterion
...
def test_complete_box_on_empty_record_is_full_independence():
    """The hole this world exists to put on the prevention floor."""
    ...
    assert independent_for(left, right, ErrorClass.FABRICATION)
...
def test_runner_pins_hold_and_refuse_changed_inputs(tmp_path):
    ...
    from experiments.aid1.run_confirmatory import PINNED, load_config, verify_pins
    ...
    target = tmp_path / "experiments/aid1/world.py"
    target.write_text(target.read_text() + "\n# changed\n")
    with pytest.raises(ValueError, match="aid1/world.py"):
