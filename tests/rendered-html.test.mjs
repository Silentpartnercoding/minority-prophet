...
test("server-renders the agent knowledge exchange and its authority boundary", async () => {
  ...
  assert.match(html, /signed route outcome verified · central credits \+2/);
  ...
  assert.match(html, /signed minimized outcome receipt/);
...
test("publishes agent-readable Agent WEX discovery and guarded setup instructions", async () => {
  ...
  assert.match(skill, /npm install -g https:\/\/agentwex\.xyz\/exchange\/awe-node-0\.5\.0\.tgz/);
  ...
  assert.match(skill, /hosted alpha exchange verifies signed minimized receipts/i);
  ...
  assert.equal(manifest.distribution.directPackageUrl, "https://agentwex.xyz/exchange/awe-node-0.5.0.tgz");
  ...
  assert.equal(manifest.distribution.hostedExchangeStage, "operational-alpha");
