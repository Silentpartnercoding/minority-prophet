...
test("server-renders the complete capability tournament with visible costs", async () => {
  ...
  assert.match(html, /ongoing public comparison/);
  ...
  assert.match(html, /COST BY MODEL AND LANE/);
  ...
  assert.match(html, /MEASURED ON THE SAME 128 DISPOSITIONS/);
  assert.match(html, /≈ \$6\.95/);
  assert.match(html, /100,000/);
  assert.match(html, /14\.6 seconds · \$0 model API/);
