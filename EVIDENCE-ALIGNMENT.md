...
| DRI-9-V1 | On 36,000 reversible synthetic decisions where dependence is recorded at no cut, belief that overrides the record's identities moves hundreds of decisions per cell, and planting a marker upstream prevented 34–77% of the errors that could turn on it while raising correct settlements from 1,273 to 1,622 in the trio family. Its decoy-family result is NOT a harm test: that family plants no markers at all (0 emitted across 768 decisions on both salts), so bait never acted there; see results/dri9-v1/POST-RESULT-NOTE.md. | That the winning instrument is established — bait was a reported arm and declaring it now would be choosing after the outcome; that combining weak signals helps, which the named method's 1-of-8 result contradicts; anything about irreversible decisions; real-world pickup, leak or cut-response rates; that an upstream can be probed at all; independent validation; authority to act. |
