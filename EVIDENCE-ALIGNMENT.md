...
| DRI-8-V1 | On 144,000 synthetic decisions with dependence recorded at no cut, active probing converted 80 of 2,281 silent false settlements into flagged ones in the pair family and 10 of 3,084 in the trio, significant in all 8 powered comparisons; probes cost 16–468 per error prevented and a larger budget bought a worse rate; passive history did nothing in the trio family and needed full feedback elsewhere. | That the protection is worth its cost — the criterion required significance and set no minimum effect; the trio family's margin split, which is mis-specified; real-world tracer detection or coincidence rates; that an upstream can be probed at all; independent validation; authority to act. |
| DRI-9-V1 | On 36,000 reversible synthetic decisions where dependence is recorded at no cut, belief that overrides the record's identities moves hundreds of decisions per cell, and planting a marker upstream prevented 34–77% of the errors that could turn on it while raising correct settlements from 1,273 to 1,622 in the trio family. Its decoy-family result is NOT a harm test: that family plants no markers at all (0 emitted across 768 decisions on both salts), so bait never acted there; see results/dri9-v1/POST-RESULT-NOTE.md and DRI-10. | That the winning instrument is established — bait was a reported arm and declaring it now would be choosing after the outcome; that the decoy tested harm — it planted zero markers, so bait's zero-merge pass is the instrument unplugged; that combining weak signals helps, which the named method's 1-of-8 result contradicts; anything about irreversible decisions; real-world pickup, leak or cut-response rates; that an upstream can be probed at all; independent validation; authority to act. |
| DRI-10-V1 | On 48,000 reversible synthetic decisions across four families, a mark is not dependence: bait collapsed a shared library that carries no shared error, lost correct settlements wherever marks leaked, and prevented 0 of 1,005 and 0 of 988 where the dependence carried no mark. Refusing when the answer is fragile prevented 473–521 in every hidden family without any instrument. | That any arm works — none passed, and the per-arm table is counterfactual relabelling; real-world pickup, leak or library-sharing rates; anything about irreversible decisions; independent validation, since the adversarial world shares the control domain; authority to act. |
| DRI-11-V1 | On a world written by the adversarial review, corroboration is not independence and fragility is not evidence: one external shock satisfied both of the composite's signals (342–439 false merges, zero true), and refusal — which needs no instrument — refused every one of 2,400 decisions a correct baseline had settled. Joint-error rate was 0.58 for the independent shocked pair against 0.38–0.42 for genuinely dependent pairs. | That any rule in this space works; that the shocked pair ought to count as dependence, which is a modelling question this raises and does not answer; real-world shock, mark or leak rates; independent validation; authority to act. |
| AID-1-V1 | On 8,100 synthetic campaigns, requiring attested witness depth prevents 177 of 350 silent false settlements in the hidden-source case at full adoption and at no cost in correct settlements, while costing all 360 correct settlements at every adoption rate where witnesses cannot attest, losing all protection one error class up as adoption rises (357, 261, 140, 98, 0), and settling 720 of 720 minority decisions against a true contrary claim at full adoption. | That the policy is usable as scoped — criterion 5 fails outright and the scope guard cannot hold, being caller-declared; that a repaired policy would fail, since two disclosed defects were deliberately left in place; that attestation is obtainable or that anyone would supply it; any real-world adoption rate or cost, since adoption is a dial in a generator; independent validation, since the world shares the control domain with the policy; authority to act. |
| AID-2-V1 | On 11,700 synthetic campaigns, reporting the range the record supports and settling only when both ends agree removes the suppression a single count caused — the true contrary claim survives 360 of 360 where the point estimate deleted all 360 — and does nothing against a decoy recorded kinship, where bounds and point are identical at every adoption rate while the baseline holds 360; against three device-attested copies of one fabricator the method prevents 0 of 360. | That the bounds idea is refuted — two of the three disclosed defects are implementation errors in the upper bound and in the corner rule, not properties of the idea; that a repaired method would fail, since all three were deliberately left in place; that depth attestation bears on shared origin, which this measures and finds it does not; real-world adoption or cost, since adoption is a generator dial; independent validation, since the world shares the control domain; authority to act. |
| AID-3-V1 | On 7,200 synthetic campaigns in a world written by the adversarial review, the repair closes the defect that world was built around — its own construction test for the completeness short-circuit now asserts the opposite and passes — while the minority suppression is untouched and total: the policy settles against a true contrary claim in 4,800 of 4,800 decisions at every adoption rate, where every other arm abstains and the claim survives. | That requiring attested depth can be made minority-safe by repair, which this refutes for the counting rule as specified; that prevention being zero means the policy is inert, when the world is constructed so collapsing the hidden pair yields a different wrong answer; that criterion 3's equal silent counts show the backing machinery does nothing, since it counts outcomes rather than mechanisms and the arms differ per decision; any real-world adoption rate, since the curve is flat by construction; independent validation, since the reviewer shares the control domain; authority to act. |
| AID-4-V1 | On 7,200 synthetic campaigns, neither remaining policy survives: a collapse-robust margin rule vindicates a true minority claim 0 times where the baseline vindicates it 3,600 of 3,600, and prevents nothing against backed copies of one hidden parent; and a published exposure figure ranks silently false settlements above correct ones at AUC 0.500–0.513 against a 0.70 floor, anti-correlated when pooled at 0.31, 0.25 and 0.19. | That any counting-time rule of this kind remains untried — both frozen kill criteria fire, which closes rules applied downstream of a record that omits what they need, and says nothing against the recorded-dependence method; that the margin rule is safe because it never settles against a minority, which conflates not being convicted with being believed and is the disclosed defect in criterion 4; that exposure is worth publishing as a warning, which its anti-correlation refutes; that a credential bears on shared origin, which two experiments now measure it does not; real-world adoption rates, since adoption is a generator dial; independent validation, since the world's author shares the control domain; authority to act. |
