...
| DRI-2-V1 | Canonical critical-path experiment: rejected | `results/dri2-v1/canonical-manifest.json` | Had as few false settlements as always escalating and far fewer than any fixed cut, with almost no excess human calls, but failed its frozen time criterion; synthetic worlds, same control domain, no held-back worlds. |
| DRI-2-V2 | Canonical critical-path experiment: supported | `results/dri2-v2/canonical-manifest.json` | Unchanged v1 method on fresh worlds with no human had as few false settlements as always abstaining and far fewer than any fixed cut, with almost no unneeded abstentions; criterion fixed after v1, synthetic worlds, same control domain, no held-back worlds. |
| DRI-3-V1 | Canonical stacked-dependence immunity experiment: supported | `results/dri3-v1/canonical-manifest.json` | Under the tiered cost rule, settling only on settlements robust to recorded possible dependence made zero silent and zero irreversible false settlements in six recorded-dependence families (bounds below 1 in 1,000) where the agreement rule settled falsely 780 times; costs looks where lineage exists and settlements where it does not; unrecorded dependence stays undetectable; the side-asymmetric family did not produce its case; synthetic, same control domain. |
| DRI-4-V1 | Canonical incomplete-record experiment: rejected | `results/dri4-v1/canonical-manifest.json` | With a complete record the tiered rule made zero silent false settlements in every family, including a trap the agreement rule failed on every decision; as true shared identities go missing its advantage erodes (33–87% of silent errors prevented at 10% missing, 10–26% at 50%) without ever becoming worse, but two powered comparisons at 10% missing were not significant; synthetic degradation model, same control domain. |
| DRI-5-V1 | Canonical content-backstop experiment: rejected | `results/dri5-v1/canonical-manifest.json` | Counting an exact content fingerprint as possible dependence removed every silent false settlement where missing lineage hid copying, recovered partially where it hid a shared component or origin, and was largely defeated when half of copies were paraphrased; 4 of 32 powered recovery comparisons were not significant; synthetic content model, same control domain. |
| DRI-6-V1 | Canonical imperfect-lookup experiment: supported | `results/dri6-v1/canonical-manifest.json` | With erring lookups every silent false settlement the tiered rule made came after a look; looking twice and settling only when both reports agree removed 68–100% of them in every powered comparison, cheaply against missed dependence and at a real cost in unneeded abstentions against invented dependence; checking a report against the record caught no missed dependence; errors independent across calls, synthetic, same control domain. |
| DRI-8-V1 | Canonical intervention experiment: supported, with a criterion that set no minimum effect | `results/dri8-v1/canonical-manifest.json` | Probing dependence the record does not carry prevented silent false settlements no record-only rule can prevent, but only 0.3–3.5% of the baseline's, at 16–468 probes each, with correct settlements unchanged; passive co-error history was weaker and merged independents far more often; the margin-critical endpoint is mis-specified for the trio family and disclosed as unmeasured there. |
| DRI-9-V1 | Canonical intervention experiment: rejected | `results/dri9-v1/canonical-manifest.json` | The method named before the run, a ladder requiring two signals to agree, cleared its 25% effect floor in 1 of 8 powered cells: most precise arm (0–6 false merges, no correct settlement lost) and least useful. Bait, a reported arm, cleared it in 8 of 8 at 34–77%, though its decoy-family zero is not a harm test: that family emits no markers at all (0 across 768 decisions on both salts), so the instrument was unplugged there; see the post-result note and DRI-10. Reflection cleared the floor while making 6,497–8,782 false merges per cell. Reversible decisions only; synthetic; same control domain. |
| DRI-10-V1 | Canonical adversarial-world experiment: rejected | `results/dri10-v1/canonical-manifest.json` | Bait, named before the run on a world written by the review of DRI-9, believed a shared library was a shared source in all four common_carrier cells (correct settlements 2,830–2,866 → 1,389–2,194, zero true merges) and prevented nothing where dependence carried no mark. No arm passes this criterion; per-arm figures in the write-up are relabelling, not verdicts. Reversible decisions only; synthetic; same control domain. |
| DRI-11-V1 | Canonical two-method experiment: rejected | `results/dri11-v1/canonical-manifest.json` | Both methods named before the world existed; both failed. Refusal fires on fragility alone: 0 of 925–961 critical errors prevented on trios, and all 2,400 decisions refused in a family the baseline settles correctly. The composite's two signals were satisfied by one shock: 342–439 false merges with zero true ones. Behaviour does not separate a shared source from a shared shock. Reversible decisions only; synthetic; same control domain. |
