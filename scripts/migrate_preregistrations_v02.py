...
def migrate(kernel_id: str) -> dict:
    ...
    if v01.get("schema") not in ("minority-prophet.preregistration.v0.1",
                                 "minority-prophet.preregistration.v0.2"):
        raise SystemExit(f"{kernel_id}: unexpected schema {v01.get('schema')!r}")
    # Idempotent: on a v0.2 input the carried fields hold either the original
    # v0.1 scalars or unanswered objects; carry() passes both through, so
    ...
    doc = {
        ...
        "statusNote": "the registered vocabulary for a seeded kernel (enforced by test_no_experiment_claims_progress_without_the_evidence_for_it: a seeded experiment must not look preregistered). The v0.2 migration is recorded in migrationRecord, not in status.",
        "migrationRecord": {
...
def update_status(kernel_id: str) -> None:
    ...
    for blocker in status["blockers"]:
        if blocker.startswith("DISCHARGED"):
            new_blockers.append(blocker)
        elif any(m in blocker for m in SCHEMA_BLOCKER_MARKERS):
            discharged += 1
            new_blockers.append(
